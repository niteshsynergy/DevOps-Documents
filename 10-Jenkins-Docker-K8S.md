...
```
eksctl create cluster --name niteshsynergy-cluster --region ap-south-1 --node-type t2.medium  --zones ap-south-1a,ap-south-1b
```
...
pipeline {
    ...
    stages {
        stage('Clone Repo') {
            steps {
                git 'https://github.com/niteshsynergy/maven-web-app.git'
            }
        }
        stage('Maven Build') {
            steps {
        ...
        stage('Docker Image') {
            steps {
                sh 'docker build -t niteshsynergy/mavenwebapp .'
            }
        }
